...
.on('end', async () => {
  ...
  for (let i = 0; i < resultado.length; i++) {
    ...
    const registro = await Registro.create({ date: data, day: parseInt(dia), month: parseInt(mes), year: parseInt(ano), weekday: parseInt(diaSemana), time: hora, city: resultado[i]['City'], state: resultado[i]['State'], country: resultado[i]['Country'], shape: resultado[i]['Shape'] });
  }
...
/*
  ...
  db.registros.find().count()
  db.registros.find().sort({ shape: 1})
  db.registros.aggregate([{$group: {_id:"$state", count: {$sum: 1}}}])
  db.registros.find({city: "Phoenix"})
  db.registros.find({state: 'CA'}, {_id: 0})
*/
